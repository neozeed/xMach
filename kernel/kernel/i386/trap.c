...
#if 0   
        printf("About to dispatch exception, user eip is %X "
	       "exc %d code %d subcode is %X, *eip %X\n", regs->eip, 
	       exc, code, subcode, 
	       inst_fetch(regs->eip, regs->cs));
        printf("uesp %x type %d/%d\n", regs->uesp, type, regs->trapno);
#endif   
