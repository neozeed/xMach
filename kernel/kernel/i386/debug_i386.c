...
static void dump_address(unsigned int addr)
{
   extern unsigned int start;
   extern unsigned int end;
   
   if (addr >= (unsigned int)&start &&
       addr <= (unsigned int)&end)
     {
	printf("%x ", addr);
     }
}

static void dump_stack(unsigned int* esp)
{
   unsigned int i;
   
   for (i = 0; i < 16; i++)
     {
	dump_address(esp[(i*4)+0]);
	dump_address(esp[(i*4)+1]);
	dump_address(esp[(i*4)+2]);
	dump_address(esp[(i*4)+3]);
     }
}
...
void dump_ss(struct i386_saved_state *st)
{
	...
	printf("trapno %d: %s, error %08x\n",
		...
		st->err);
        dump_stack((unsigned int *)st->uesp);
}
